...
int main()
{
    ...
    Camera cameraInst(&shader, width, height);
	camera = &cameraInst;
...
// Função callback acionada quando há movimento de mouse
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (camera)
...
// Função callback acionada quando há movimento de scroll
void scroll_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (camera)
		camera->scrollCamera(ypos);
}
